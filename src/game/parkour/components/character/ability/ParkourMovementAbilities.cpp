...
#undef VAULT_FAIL
		...
		public:
			...
			bool Start(MovementContext& context, float) override {
				...
				runtime.blink.cooldown = mConsole ?
					mConsole->GetConVarValueOr("park_blink_cooldown", 1.0f) : 1.0f;
				context.isGrounded = false;
				context.supportEntityGuid = 0;
				context.supportLinearVelocity = Vec3::zero;
				context.supportStepDelta = Vec3::zero;
			...
			bool Tick(MovementContext& context, float deltaTime) override {
				...
				const float duration = std::max(
					1.0e-4f,
					mConsole ? mConsole->GetConVarValueOr("park_blink_moveduration", 0.08f) : 0.08f
				);
				runtime.blink.moveTime += std::max(0.0f, deltaTime);
				const float t = std::clamp(runtime.blink.moveTime / duration, 0.0f, 1.0f);
				context.transform->SetPosition(
					Math::Lerp(runtime.blink.startPos, runtime.blink.targetPos, t)
				);
		...
		public:
			...
			bool Start(MovementContext& context, float) override {
				...
				const float startBoost = mConsole ?
					                         ...
					                         50.0f;
				context.velocity += runtime.wallRun.direction * Math::HtoM(startBoost);
				context.modeTickSuppressed = true;
			...
			bool Tick(MovementContext& context, float deltaTime) override {
				...
				if (detachOnSideInput && std::abs(context.input.moveAxis.x) > 0.5f) {
					const Vec3 camRight = HorizontalNormalized(context.input.right);
					if (!camRight.IsZero()) {
						const float wallSide = camRight.Dot(runtime.wallRun.normal);
						if (
				...
				if (context.input.jumpPressed) {
					if (!runtime.wallRun.jumpWasHeldOnInit) {
						...
						runtime.hasDoubleJump = true;
						parkour->PublishParkourMovementCue("movement.wallrun.jump");
						context.jumpSnapDisableRemaining = std::max(
							context.jumpSnapDisableRemaining,
							mConsole ? mConsole->GetConVarValueOr("sv_jumpsnapdisabletime", 0.1f) : 0.1f
						);
				...
				const float gravity = mConsole ? mConsole->GetConVarValueOr("sv_gravity", 800.0f) : 800.0f;
				const float gravityScale = mConsole ? mConsole->GetConVarValueOr("park_wallrun_gravityscale", 0.1f) : 0.1f;
				...
				const float wishSpeed = mConsole ?
					                        ...
					                        320.0f;
				if (context.input.moveAxis.z > 0.0f && !runtime.wallRun.direction.IsZero()) {
		...
		public:
			void Init(ConsoleSystem* console) override { mConsole = console; }
			[[nodiscard]] MOVEMENT_ABILITY_SLOT GetSlot() const override { return MOVEMENT_ABILITY_SLOT::SLIDE; }
			[[nodiscard]] std::string_view GetDebugName() const override { return "parkour_slide"; }
			[[nodiscard]] bool IsForcedAbility() const override { return false; }
			[[nodiscard]] bool CanRunInMode(MOVEMENT_MODE_ID modeId) const override { return modeId == MOVEMENT_MODE_ID::GROUND; }
			[[nodiscard]] bool CanStart(const MovementContext& context) const override {
				const auto* parkour = dynamic_cast<const ParkourMovementComponent*>(context.movementComponent);
				if (!parkour || !context.input.crouchPressed || context.modeState.currentMode != MOVEMENT_MODE_ID::GROUND) {
					return false;
				}
				return parkour->ShouldSlideFromSpeed(parkour->GetHorizontalSpeedHu(context.velocity));
			}
			bool Start(MovementContext& context, float) override {
				auto* parkour = AsParkour(context);
				if (!parkour) {
				...
				float boostedSpeed = horizontalVelocity.Length() + Math::HtoM(
					mConsole ?
						mConsole->GetConVarValueOr("park_slide_boostspeed", 50.0f) :
						50.0f
				);
				const float speedCap = Math::HtoM(
					mConsole ?
						mConsole->GetConVarValueOr("park_slide_hopspeedcap", 5000.0f) :
						5000.0f
				);
				boostedSpeed = std::min(
					boostedSpeed,
					speedCap > 0.0f ? speedCap : std::numeric_limits<float>::max()
				);
				const float originalY = context.velocity.y;
			...
			bool Tick(MovementContext& context, float deltaTime) override {
				...
				ApplyGroundFriction(
					context.velocity,
					mConsole ? mConsole->GetConVarValueOr("park_duckfriction", 0.75f) : 0.75f,
					deltaTime,
					mConsole
				);
				GroundAccelerate(
					context.velocity,
					wishDir,
					mConsole ? mConsole->GetConVarValueOr("sv_duckspeed", 63.3f) : 63.3f,
					mConsole ? mConsole->GetConVarValueOr("park_duckaccelerate", 4.0f) : 4.0f,
					deltaTime,
					mConsole
				);
				...
				if (context.input.jumpPressed) {
					runtime.slide.active = false;
					ExecuteGroundJumpAndSwitchToAir(context, mConsole, deltaTime);
					return false;
				}

				Vec3 movePos = context.transform ? context.transform->GetPosition() :
					               Vec3::zero;
				Vec3 moveVel = context.velocity;
				context.resolver->StepMove(
					movePos,
					moveVel,
					mConsole ? mConsole->GetConVarValueOr("sv_stepheight", 18.0f) : 18.0f,
					deltaTime
				);
				...
				if (groundNormal.y < 0.999f && groundNormal.y > 0.0f) {
					const float gravityM = Math::HtoM(
						mConsole ? mConsole->GetConVarValueOr("sv_gravity", 800.0f) : 800.0f
					);
				...
				const float maxSlideTime = mConsole ?
					                           mConsole->GetConVarValueOr("park_slide_maxtime", 20.0f) :
					                           20.0f;
				...
				const float stopSpeedHu = mConsole ?
					                          mConsole->GetConVarValueOr("park_slide_stopspeed", 50.0f) :
					                          50.0f;
		...
		public:
			void Init(ConsoleSystem* console) override { mConsole = console; }
			[[nodiscard]] MOVEMENT_ABILITY_SLOT GetSlot() const override { return MOVEMENT_ABILITY_SLOT::SPEED_VAULT; }
			[[nodiscard]] std::string_view GetDebugName() const override { return "parkour_speedvault"; }
			...
			[[nodiscard]] bool CanStart(const MovementContext& context) const override {
				...
				const auto& runtime = parkour->GetParkourRuntime();
				if (runtime.vault.active || runtime.vault.cooldown > 0.0f || context.isGrounded) {
					return false;
				}
				if (context.input.crouchPressed || context.input.moveAxis.z <= 0.5f) {
					return false;
				}
				if (parkour->IsDuckHullEnabled() && !parkour->CanStandAt(context)) {
					return false;
				}
				SpeedVaultTrajectory trajectory = {};
				return TryBuildSpeedVaultTrajectory(context, mConsole, trajectory);
			}
			bool Start(MovementContext& context, float) override {
				auto* parkour = AsParkour(context);
				if (!parkour) {
				...
				SpeedVaultTrajectory trajectory = {};
				if (!TryBuildSpeedVaultTrajectory(context, mConsole, trajectory)) {
					return false;
				}
				...
				context.supportStepDelta = Vec3::zero;
				context.modeTickSuppressed = true;
				context.SubmitTransition(MOVEMENT_MODE_ID::AIR, MOVEMENT_TRANSITION_PRIORITY::FORCED_ABILITY, "vault start", "ParkourSpeedVaultAbility");
				return true;
			}
			bool Tick(MovementContext& context, float deltaTime) override {
				auto* parkour = AsParkour(context);
				...
				context.modeTickSuppressed = true;
				const float duration = std::max(1.0e-4f, mConsole ? mConsole->GetConVarValueOr("park_vault_duration", 0.25f) : 0.25f);
				runtime.vault.time += std::max(0.0f, deltaTime);
				const float t = std::clamp(runtime.vault.time / duration, 0.0f, 1.0f);
				const float u = 1.0f - t;
				context.transform->SetPosition(runtime.vault.startPos*(u*u) + runtime.vault.apexPos*(2.0f*u*t) + runtime.vault.endPos*(t*t));
				parkour->SyncCollisionHull(context.transform);
				context.velocity = Vec3::zero;
				context.isGrounded = false;
				context.supportEntityGuid = 0;
				...
				if (t >= 1.0f) {
					...
					runtime.vault.cooldown = mConsole ? mConsole->GetConVarValueOr("park_vault_cooldown", 0.3f) : 0.3f;
					...
					const float minExitSpeed = Math::HtoM(
						mConsole ? mConsole->GetConVarValueOr("park_vault_minspeed", 150.0f) : 150.0f
					);
					if (horizontalSpeed < minExitSpeed) {
						horizontalSpeed = minExitSpeed;
					}

					Vec3 vaultDir = runtime.vault.endPos - runtime.vault.startPos;
					vaultDir.y = 0.0f;
					if (!vaultDir.IsZero()) {
						vaultDir.Normalize();
					} else {
					...
					context.isGrounded = true;
					runtime.hasDoubleJump = true;
					context.SubmitTransition(MOVEMENT_MODE_ID::GROUND, MOVEMENT_TRANSITION_PRIORITY::FORCED_ABILITY, "vault end", "ParkourSpeedVaultAbility");
					return true;
				}
				return true;
			}
		...
		public:
			void Init(ConsoleSystem* console) override { mConsole = console; }
			[[nodiscard]] MOVEMENT_ABILITY_SLOT GetSlot() const override { return MOVEMENT_ABILITY_SLOT::GRAPPLE; }
			[[nodiscard]] std::string_view GetDebugName() const override { return "parkour_grapple"; }
			[[nodiscard]] bool IsForcedAbility() const override { return false; }
			[[nodiscard]] bool CanRunInMode(MOVEMENT_MODE_ID) const override { return true; }
			[[nodiscard]] bool CanStart(const MovementContext& context) const override {
				const auto* parkour = dynamic_cast<const ParkourMovementComponent*>(context.movementComponent);
				return parkour && parkour->GetActionFrameInput().weapon.secondary.pressed;
			}
			bool Start(MovementContext& context, float) override {
				if (auto* parkour = AsParkour(context)) {
	...
	void RegisterParkourMovementAbilities(GameMovementStateMachine& stateMachine) {
