...
void main(uint3 DTid : SV_DispatchThreadID) {
	...
	if (vertexIndex >= inputCount) { return; }
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (weight > 0.0f && boneIndex < 256) {
			...
			skinnedNormal += mul(
				inputVertex.normal, (float3x3)boneMatrix
			) * weight;
		}
	}
	...
	outputVertex.normal   = normalize(
		mul(
			skinnedNormal, (float3x3)gTransformationMatrix.WorldInverseTranspose
		)
	);
	outputVertex.worldPosition = mul(
		skinnedPosition, gTransformationMatrix.World
	).xyz;
