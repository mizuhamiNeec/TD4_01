...
namespace Unnamed {
	...
	/// @brief タイトル演出（demo再生/フェード/開始遷移）を制御するコンポーネントです。
	...
	private:
		...
		/// @brief タイトルモード中かどうかを判定します。
		[[nodiscard]] bool IsTitleModeEnabled() const;
		/// @brief タイトルモード開始時の初期化を行います。
		void EnterTitleMode();
		/// @brief タイトルモード終了時の後始末を行います。
		void ExitTitleMode();
		...
		/// @brief 指定ConVarを秒値として取得します。
		[[nodiscard]] float GetSecondsCvar(
		...
		std::string mPlayPromptName    = "PlayPromptImage";
