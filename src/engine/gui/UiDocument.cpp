...
namespace Unnamed::Gui {
	...
	void UiDocument::SetName(const std::string& name) { mName = name; }

	const std::string& UiDocument::GetName() { return mName; }
	...
	UiWidget* UiDocument::GetRootWidget() const { return mRootWidget.get(); }
	...
	void UiDocument::RegisterWidgetRecursive(UiWidget* widget) {
		...
		if (!name.empty()) { RegisterNamedWidget(std::string(name), widget); }
