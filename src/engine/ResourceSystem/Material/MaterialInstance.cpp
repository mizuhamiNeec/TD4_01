...
/// @brief マテリアルインスタンスのテクスチャをSRVとして適用します
...
void MaterialInstance::Apply(
	...
	for (size_t i = 0; i < textureOrder.size(); ++i) {
		...
		if (texturePath.empty()) {
			Console::Print(
				"MaterialInstance::Apply: テクスチャパスが空です\n",
		...
		if (texturePath.find("dev_measure.png") != std::string::npos ||
			...
			Console::Print(
				std::format(
					"MaterialInstance::Apply: 追跡中のテクスチャ: {} をバインドします\n",
					texturePath
				),
				kConTextColorWarning, Channel::ResourceSystem
			);
		...
		// テクスチャのGPUハンドルを直接取得
		D3D12_GPU_DESCRIPTOR_HANDLE handle = texManager->GetSrvHandleGPU(
			texturePath
		);
