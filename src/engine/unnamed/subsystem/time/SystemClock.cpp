...
/// @return システム起動時刻
SystemClock::TimePoint SystemClock::StartTime() { return mStartTime; }

/// @brief 現在時刻を取得します
/// @return 現在時刻
SystemClock::TimePoint SystemClock::Now() { return SysClock::now(); }

/// @brief システム起動からの経過時間を秒単位で取得します
/// @return システム起動からの経過時間（秒）
double SystemClock::UpTime() {
	if (mStartTime.time_since_epoch().count() == 0) { return 0.0; }
...
/// @return 年
int SystemClock::Year(const TimePoint& tp) { return ToTm(tp).tm_year + 1900; }

/// @brief 指定したTimePointから月を取得します
/// @param tp 取得するTimePoint
/// @return 月
int SystemClock::Month(const TimePoint& tp) { return ToTm(tp).tm_mon + 1; }

/// @brief 指定したTimePointから日を取得します
/// @param tp 取得するTimePoint
/// @return 日
int SystemClock::Day(const TimePoint& tp) { return ToTm(tp).tm_mday; }

/// @brief 指定したTimePointから時を取得します
/// @param tp 取得するTimePoint
/// @return 時
int SystemClock::Hour(const TimePoint& tp) { return ToTm(tp).tm_hour; }

/// @brief 指定したTimePointから分を取得します
/// @param tp 取得するTimePoint
/// @return 分
int SystemClock::Minute(const TimePoint& tp) { return ToTm(tp).tm_min; }

/// @brief 指定したTimePointから秒を取得します
/// @param tp 取得するTimePoint
/// @return 秒
int SystemClock::Second(const TimePoint& tp) { return ToTm(tp).tm_sec; }
