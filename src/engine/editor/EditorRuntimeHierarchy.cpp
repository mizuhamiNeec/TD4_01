...
namespace Unnamed {
	namespace {
		...
		void SortComponentMenuTree(ComponentMenuNode& node) {
			std::ranges::sort(
				...
				const ComponentRegistry::RegisteredComponentInfo& rhs
					...
					const std::string_view lhsLabel = lhs.displayName.empty() ?
						lhs.stableName :
						lhs.displayName;
					const std::string_view rhsLabel = rhs.displayName.empty() ?
						rhs.stableName :
						rhs.displayName;
		...
		bool DrawComponentAddMenuRecursive(
			...
			for (const auto& [scopeName, childNode] : node.children) {
				...
				if (ImGui::BeginMenu(scopeName.c_str())) {
			...
			for (const auto& info : node.components) {
				...
				const std::string visibleLabel = info.displayName.empty() ?
					                                 info.stableName :
					                                 info.displayName;
				const std::string label = visibleLabel + "###" + info.
				                          stableName;
				const bool alreadyExists = existingStableNames.contains(
					info.stableName
				);
				if (
					ImGui::MenuItem(
						label.c_str(), nullptr, false, !alreadyExists
					)
				) {
	...
	void EditorRuntime::DrawSceneOutliner() {
	...
	void EditorRuntime::DrawInspector() {
		...
		if (
			ImGuiWidgets::IconButton(
				kIconAdd, "AddComponent", ImVec2(0.0f, 0.0), 1.5f,
				ImGuiDir_Right
			)
		) {
		...
		if (ImGui::BeginPopup("InspectorAddComponentPopup")) {
			...
			if (validComponentCount == 0) {
				ImGui::TextUnformatted("No registered components.");
			} else if (
