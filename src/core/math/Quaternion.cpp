...
Quaternion Quaternion::AxisAngle(const Vec3& axis, const float angleRad) {
...
Vec3 Quaternion::operator*(const Vec3& vec) const {
	...
	return Vec3(
		vec.x * (1.0f - (tyy + tzz)) + vec.y * (txy - twz) + vec.z * (txz +
			twy),
		vec.x * (txy + twz) + vec.y * (1.0f - (txx + tzz)) + vec.z * (tyz -
			twx),
