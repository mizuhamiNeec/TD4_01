...
float4 WideGaussianBlur(
	Texture2D tex, SamplerState samp, float2 uv, float2 texelSize, float radius
) {
...
float4 HighQualityBloom(
	...
	float4 bloom = (b0 * 0.5 + b1 * 0.35 + b2 * 0.2 + b3 * 0.1) / (
		               0.5 + 0.35 + 0.2 + 0.1);

	float lum = GetLuminance(bloom.rgb);
	if (lum < threshold) { bloom = float4(0, 0, 0, 0); }
	return bloom;
}
...
float4 main(VertexShaderOutput input) : SV_TARGET {
	...
	float4 bloom = HighQualityBloom(
		gTexture, gSampler, input.texcoord, texelSize, bloomThreshold
	);
