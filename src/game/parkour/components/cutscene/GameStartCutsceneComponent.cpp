...
namespace Unnamed {
	...
	void GameStartCutsceneComponent::OnDetached() {
		...
		mHasSavedGameplayPose  = false;
		mHasCountdownFixedPose = false;
	...
	void GameStartCutsceneComponent::DrawInspectorImGui() {
		ImGui::InputScalar(
			"Camera Root Entity Guid",
			ImGuiDataType_U64,
			&mCameraRootEntityGuid
		);
		ImGui::InputScalar(
			"Camera Rotator Component Guid",
			ImGuiDataType_U64,
			&mCameraRotatorComponentGuid
		);
		ImGui::InputScalar(
	...
	void GameStartCutsceneComponent::Deserialize(const JsonReader& reader) {
		if (const JsonReader node = reader["cameraRootEntityGuid"]; node.Valid()) {
			mCameraRootEntityGuid = node.GetUint64();
		}
		if (const JsonReader node = reader["cameraRotatorComponentGuid"];
			node.Valid()) {
			mCameraRotatorComponentGuid = node.GetUint64();
		}
		...
		mCountdownDigitWidgetName = reader["countdownDigitWidgetName"].GetString(
			mCountdownDigitWidgetName
		);
		mCountdownStartWidgetName = reader["countdownStartWidgetName"].GetString(
			mCountdownStartWidgetName
		);
		mFadeOverlayWidgetName = reader["fadeOverlayWidgetName"].GetString(
		...
		if (const JsonReader lockNode = reader["lockTargets"]; lockNode.Valid()) {
			...
			for (size_t i = 0; i < lockArray.Size(); ++i) {
				...
				spec.componentStableName = item["componentStableName"].GetString(
	...
	void GameStartCutsceneComponent::Serialize(JsonWriter& writer) const {
		...
		writer.Key("cameraRotatorComponentGuid");
		writer.Write(mCameraRotatorComponentGuid);
	...
	void GameStartCutsceneComponent::StartSequence() {
		...
		// Demo再生時は演出を完全にスキップし、入力ロックも行いません。
		if (DemoManager* demo = GetDemoManager(); demo && demo->IsPlayback()) {
			mHasSavedGameplayPose = false;
			EnterGameplay();
			return;
		}
		...
		if (mCameraRootTransform) {
			mSavedGameplayCameraPosition = mCameraRootTransform->GetPosition();
			mHasSavedGameplayPose        = true;
		}
		if (mCameraRotator) {
			mSavedGameplayLookAngles = mCameraRotator->GetLookAnglesDegrees();
			mHasSavedGameplayPose    = true;
		} else if (mCameraRootTransform) {
			const Vec3 euler = mCameraRootTransform->GetRotation().ToEulerDegrees();
			mSavedGameplayLookAngles = Vec2(euler.x, euler.y);
			mHasSavedGameplayPose    = true;
		}
		...
		ApplyCameraPose(firstShot.startPos, firstShot.startLook);
		UpdateHudWidgets();
	}
	...
	void GameStartCutsceneComponent::TickTour(const float deltaTime) {
		...
		ApplyCameraPose(cameraPos, lookAtPos);
		...
		if (rawT >= 1.0f) {
			if (hasNextShot) {
				...
				ApplyCameraPose(nextShot.startPos, nextShot.startLook);
			} else {
				mFadeAlpha = 0.0f;
				EnterCountdown();
				return;
	...
	void GameStartCutsceneComponent::TickCountdown(const float deltaTime) {
		...
		const float totalDuration = digitTotalDuration + std::max(
			kMinDurationSec,
			mCountdownStartSeconds
		);
		...
		if (
			!mGameplayControlReleased &&
			mCountdownElapsedSeconds >= digitTotalDuration
		) {
			RestoreLockTargets();
			mHasCountdownFixedPose    = false;
		...
		// 演出中の視点ぶれを防ぐため、固定姿勢を毎フレーム再適用します。
		if (mHasCountdownFixedPose) {
			ApplyLookAnglesAndPosition(
				mCountdownFixedPosition,
				mCountdownFixedLookAngles
			);
		}
	...
	void GameStartCutsceneComponent::EnterCountdown() {
		...
		// Branch挙動に合わせ、カウント開始はプレイヤー基準姿勢へ戻します。
		if (mHasSavedGameplayPose) {
			ApplyLookAnglesAndPosition(
				mSavedGameplayCameraPosition,
				mSavedGameplayLookAngles
			);
			mCountdownFixedPosition   = mSavedGameplayCameraPosition;
			mCountdownFixedLookAngles = mSavedGameplayLookAngles;
			mHasCountdownFixedPose    = true;
		} else if (mCameraRootTransform) {
			mCountdownFixedPosition = mCameraRootTransform->GetPosition();
			if (mCameraRotator) {
				mCountdownFixedLookAngles = mCameraRotator->GetLookAnglesDegrees();
			} else {
				const Vec3 euler = mCameraRootTransform->GetRotation().ToEulerDegrees();
				mCountdownFixedLookAngles = Vec2(euler.x, euler.y);
			}
			mHasCountdownFixedPose = true;
		}
	...
	void GameStartCutsceneComponent::UpdateCountdownAudio() {
		...
		if (mCountdownElapsedSeconds < digitTotalDuration) {
			const int digit = 3 - static_cast<int>(mCountdownElapsedSeconds / digitDuration);
	...
	void GameStartCutsceneComponent::UpdateCountdownWidgets() const {
		if (
			!mCountdownDigitWidget || !mCountdownDigitTransform ||
		...
		const float startScale = std::clamp(
			viewW * 0.42f / startBaseWidth,
			0.45f,
			1.45f
		);
	...
	void GameStartCutsceneComponent::ResolveBindings() {
		...
		if (mCameraRootEntityGuid != 0) {
			mCameraRootEntity = scene->FindEntity(mCameraRootEntityGuid);
		}
		if (!mCameraRootEntity) {
			mCameraRootEntity = GetOwner();
		}
		if (mCameraRootEntity) {
			mCameraRootTransform = mCameraRootEntity->GetComponent<TransformComponent>();
		}

		auto tryResolveRotatorInEntity = [&](Entity& entity) {
			CameraRotatorComponent* found = nullptr;
			entity.ForEachComponent(
				[&](BaseComponent& component) {
					if (component.GetGuid() != mCameraRotatorComponentGuid) {
						return true;
					}
					found = dynamic_cast<CameraRotatorComponent*>(&component);
					return false;
				}
			);
			return found;
		...
		if (mCameraRotatorComponentGuid != 0) {
			if (mCameraRootEntity) {
				mCameraRotator = tryResolveRotatorInEntity(*mCameraRootEntity);
			}
			if (!mCameraRotator) {
				for (const auto& entityPtr : scene->GetEntities()) {
					...
					mCameraRotator = tryResolveRotatorInEntity(*entityPtr);
					if (mCameraRotator) {
						break;
					}
				}
		...
		if (!mCameraRotator && mCameraRootEntity) {
			mCameraRotator = mCameraRootEntity->GetComponent<CameraRotatorComponent>();
		}
		...
		if (mHudCanvas && mHudCanvas->EnsureRuntimeLoaded()) {
			...
			if (rootWidget) {
				...
				if (mCountdownDigitWidget) {
					mCountdownDigitTransform = mCountdownDigitWidget
						->GetOrAddComponent<Gui::UiTransformComponent>();
					mCountdownDigitTexture = mCountdownDigitWidget
						->GetOrAddComponent<Gui::UiTextureComponent>();
				}
				if (mCountdownStartWidget) {
					mCountdownStartTransform = mCountdownStartWidget
						->GetOrAddComponent<Gui::UiTransformComponent>();
					mCountdownStartTexture = mCountdownStartWidget
						->GetOrAddComponent<Gui::UiTextureComponent>();
				}
				if (mFadeOverlayWidget) {
					mFadeOverlayTexture = mFadeOverlayWidget
						->GetOrAddComponent<Gui::UiTextureComponent>();
				}
			}
		}
	...
	void GameStartCutsceneComponent::ApplyCameraPose(
		const Vec3& cameraPos,
		const Vec3& lookAtPos
	) {
		if (!mCameraRootTransform) {
		...
		mCameraRootTransform->SetPosition(cameraPos);
		...
		mCameraRootTransform->SetRotation(lookRotation);
	...
	void GameStartCutsceneComponent::ApplyLookAnglesAndPosition(
		const Vec3& position,
		...
		mCameraRootTransform->SetPosition(position);

		const Quaternion rotation =
			Quaternion::AxisAngle(Vec3::up, lookAngles.y * Math::deg2Rad) *
			Quaternion::AxisAngle(Vec3::right, lookAngles.x * Math::deg2Rad);
		mCameraRootTransform->SetRotation(rotation);
		if (mCameraRotator) {
			mCameraRotator->SetLookAnglesDegrees(lookAngles.x, lookAngles.y);
		}
	}
